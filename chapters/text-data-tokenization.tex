...
\term{Why the exclusive upper bound is \(N-T\)}: the last valid start is
\(N-T-1\), where \(N\) is the number of tokens and \(T\) is the block size.
That start still supplies \(T+1\) tokens. Subtracting one again would discard
the last window and reject the smallest valid dataset. The executable
\texttt{test\_shift\_and\_last\_valid\_start} locks down this boundary.
