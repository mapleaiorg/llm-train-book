...
\usepackage{etoolbox}
% Keep legacy and new TikZ plates inside the printable measure, without rasterizing.
...
\hypersetup{
  ...
  pdftitle={Training Large Language Models From Scratch},
...
\title{Training Large Language Models From Scratch\\\large From tokens and gradients to industrial distributed training.}
\author{MapleAI Intelligence Inc.}
\date{Working edition: September 2026}
...
\term{Evidence boundary for this working edition.} Historical GPT21 and backend
case studies in Parts I--IV are reported instructional material; their raw run
artifacts are not bundled here and have not been reproduced in this revision.
Part V introduces the repository's executable CPU reference and its tests.
Quantitative reasoning distinguishes measured results, derived formulas,
reported observations and estimates. Neither the title nor a roadmap entry
means that distributed industrial training is already implemented.
...
\begin{itemize}
  ...
  \item Part V connects the foundations to a tested modern CPU training engine and the checkpoint boundary with inference. Distributed training remains a later implementation milestone.
\end{itemize}
...
\part{Foundations}
\input{chapters/foundations-roadmap}
\input{chapters/mathematical-foundations}
\input{chapters/text-data-tokenization}

\part{Model Mechanics}
\input{chapters/tensors-and-pytorch}
\input{chapters/embeddings-and-representations}
\input{chapters/attention-and-transformers}
\input{chapters/gpt-architecture}

\part{Training}
\input{chapters/loss-optimization-training}
\input{chapters/pretraining-engineering}
\input{chapters/evaluation-and-inference}

\part{Systems And Practice}
\input{chapters/model-files-and-runtime-buffers}
\input{chapters/hardware-and-backends}
\input{chapters/fine-tuning-and-course-projects}
\input{chapters/lab-manual}
\input{chapters/glossary}

\part{The Executable Training Engine}
\input{chapters/executable-modern-core}
\input{chapters/checkpoint-runtime-boundary}
